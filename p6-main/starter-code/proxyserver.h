...
struct parsed_http_request *parse_client_request(int fd)
{
    ...
    int delay = -1;
    // int priority = -1; removed priority
    char *path = NULL;
    ...
    struct parsed_http_request *http_req = malloc(sizeof(struct parsed_http_request));
    http_req->path = path;
    http_req->delay = delay;
